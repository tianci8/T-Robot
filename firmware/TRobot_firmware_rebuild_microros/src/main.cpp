...
void setup()
{
    ...
    set_microros_transports(); //若使用串口请使用该行，并注释掉下行
    // set_microros_wifi_transports("SSID", "password", "192.168.50.104", 8888); 若使用wifi，更改SSID和password，IP
    createEntities();
}
...
bool createEntities()
{
    ...
    // 创建Node，名称可自行更改
    RCCHECK(rclc_node_init_default(&node, "TRobot_node", "", &support));
    // 创建odometry publisher
    RCCHECK(rclc_publisher_init_best_effort(
        &odom_publisher,
        &node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(nav_msgs, msg, Odometry),
        "odom")); //若使用IMU和odom进行数据融合，使用odom/unfiltered。若单独使用odom，则为odom
    // 创建IMU publisher
    RCCHECK(rclc_publisher_init_best_effort(
        &imu_publisher,
        &node,
    ...
    // 创建定时器，100Hz的频率驱动电机(1000/10)
    const unsigned int control_timeout = 10;        // 100hZ

    // 定时器，发布odom_publisher 20 Hz (1000/50)
    const unsigned int odom_publisher_timeout = 25; // 40Hz

    // 定时器，发布imu_publisher 20 Hz (1000/50)
    const unsigned int imu_publisher_timeout = 50;  // 20Hz
    //上述定时器频率都可以按需修改
...
void moveBase()
{
    //若长时间没收到cmd_vel命令，则停止运动。或者第一次收到命令，也会停止一下。这部分代码可以按需使用
